...
def create_modules(module_defs, img_size, arc):
    ...
    #  routs是一个for循环的外部变量，for循环中的modules每创建一个层，都此层的i进行保存。在route、shortcut会使用到
...
     通过type字样不同的类型，来进行模型构建
    ...
    # 返回的是搭建完成的module_list, 和记录每层组件所在位置的routs
...
class YOLOLayer(nn.Module):
    def __init__(self, anchors, nc, img_size, yolo_index, layers):
        ...
        self.no = nc + 5  # number of outputs (85) 每个格子对应输出的维度 class + 5 中5代表x,y,w,h,conf
        self.nx = 0  # initialize number of x gridpoints 初始化x方向上的格子数量
        self.ny = 0  # initialize number of y gridpoints 初始化y方向上的格子数量
...
     在这里ONNX代表规范化的推理过程
        ...
        '''
        原先的P是由上一层卷积得到的feature map, 形状为(以80个类别、输入416、下采样32倍为例)：【batch size, anchor×(80+5), 13, 13】，在
        训练的过程中，将feature map通过张量操作转化的形状为：【batch size, anchor, 13, 13, 85】
        '''
        if self.training:
            return p
        ...
        # p的形状目前为：【bs, anchor_num, gridx,gridy,xywhc+class】
            ...
            io = p.clone()  # inference output 测试过程输出就是io
            # grid_xy是左上角再加上偏移量io[...:2]代表xy偏移
            # 最终边界框的坐标bx,by = sigmoid(xy的偏移量) + 网格的xy
            io[..., :2] = torch.sigmoid(io[..., :2]) + self.grid_xy  # xy
            # p_w, p_h代表的是anchor先验框在feature map上对应的大小。
            # t_w, t_h代表的是网络学习得到的缩放系数
            # 最终预测边界的bw,bh = pw，ph * exp（tw,th）
            io[..., 2:4] = torch.exp(io[..., 2:4]) * self.anchor_wh  # wh yolo method
            # 乘以缩放系数，将预测的bbox还原到输入尺寸大小
            io[..., :4] *= self.stride
            torch.sigmoid_(io[..., 4:])
...
class Darknet(nn.Module):
    ...
    def __init__(self, cfg, img_size=(416, 416), arc='default'):
        ...
        '''
        参数1：self.module_defs
        调用了 parse_model_cfg 函数，得到了 module_defs 对象。实际上该函数是通过解析cfg文件，得到
        一个list，list中包含多个字典，每个字典保存的内容就是一个模块内容，
        '''
    ...
    # 构建网络前向传播过程
    '''
    forward的过程也比较简单，通过得到的module_defs和module_list变量，通过for循环将整个
    module_list中的内容进行一遍串联，需要得到的最终结果是YOLO层的输出。
    '''
    def forward(self, x, verbose=False):
        ...
        for i, (mdef, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            if mtype in ['convolutional', 'upsample', 'maxpool']:
                ...
                # route操作就是将几个层的内容拼接（concat）起来，具体可以看cfg文件解析
                layers = mdef['layers']
                if verbose:
                    l = [i - 1] + layers  # layers
...
'''
yolo layer的获取是通过解析module_defs这个存储cfg文件中的信息的变量得到的。以yolov3.cfg为
例，最终返回的是yolo层在整个module的序号。比如：第83,94,106个层是YOLO层
'''
